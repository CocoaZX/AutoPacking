...
#   __BUILD_TARGET="wormHole"
#   __SCHEME_NAME="wormHole"
# else
...
# 5.上传安装包到指定位置
__UPLOAD_IPA_OPTIONS=("1.Pgyer" "2.None")
...
# 8.是否checkout.
__GIT_CHECKOUT=("1.不清除本地代码" "2.清除本地代码")
READ_USER_INPUT "请选择是否需要清除本地未提交代码" "${__GIT_CHECKOUT[*]}" ${#__GIT_CHECKOUT[*]}
...
#git
cd ../../
if [[ $__IS_GIT_CHECKOUT -eq 2 ]]; then
  git checkout .
fi
git pull
...
if [[ __IS_BUNDLE_FAILE -eq 1 ]]; then
  #statements
  exit 1
fi
...
if test -f "${__EXPORT_IPA_PATH}/${__IPA_NAME}.ipa" ; then
  ...
  if [[ $__UPLOAD_IPA_OPTION -eq 2 ]]; then
    printMessage "您选择了不上传到内测网站"
  elif [[ $__UPLOAD_IPA_OPTION -eq 1 ]]; then
